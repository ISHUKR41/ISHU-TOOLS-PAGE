...
## Update (2026-04-25 — round 11) — Lazy-loaded 504 KB SEO chunk + registry dedup

**Two real perf/quality wins this session.**

### Frontend perf — lazy SEO database
- `ToolPage.tsx` no longer statically imports `seoData.ts` (504 KB). Replaced with a memoised `loadSeoModule()` dynamic import + tiny `fallbackToolSEO()` helper.
- All 6 call sites of `getToolSEO`/`getToolJsonLd`/`getFaqJsonLd` updated. The `useMemo seo` hook auto-upgrades fallback → handcrafted SEO once the chunk lands; the FAQ section reuses the memoised `seo` instead of re-calling `getToolSEO`.
- `await fetchTool()` and `await loadSeoModule()` now run in parallel via `Promise.all` to save one round-trip on first navigation.
- **Result:** ToolPage chunk dropped from ~600 KB combined to **91 KB**; seo-data is now a true on-demand chunk. Crawlers/AI bots are unaffected — the 1247 prerendered HTML files (regenerated by `gen-static-seo.mjs`) ship every meta tag/JSON-LD/FAQ inline.

### Backend cleanup — registry duplicate slugs
- `registry.py` had **88 slugs declared 2-6 times each** (1065 declarations / 945 unique). Runtime was already deduping at line 7607 via `_seen_tools` dict (last-wins), but the source bloat made maintenance error-prone (editing the wrong copy = no effect).
- Wrote a paren-balanced parser, found the 120 redundant `ToolDefinition(...)` blocks, removed all but the LAST occurrence of each slug.
- **Saved 878 lines (~10% of registry.py)**; backend boot still serves all **1247 tools** with correct metadata; verified `color-palette-generator`, `tip-calculator`, `number-to-words`, `instagram-downloader` etc. all retain their full SEO description + tags.

### End-to-end verification (popular tools)
| Tool | Result |
|---|---|
| base64-encode, base64-tool, string-case-converter, color-palette-generator, instagram-bio-generator, hashtag-generator, number-to-words, tip-calculator, json-formatter, password-generator, url-encoder, word-counter, uuid-generator, lorem-ipsum-generator, regex-tester, markdown-to-html | ✓ HTTP 200 / status="success" |
| instagram-downloader | ✓ Returns graceful cookie-recovery message (Instagram blocks all anonymous cloud-IP downloads — this is the correct production behavior, not a bug) |
